...
Future<List<Tuple2<int, PostType>>> getTimeLineData(int uid) async {
  final getData = await http.get(Uri.http("https://23.254.244.168", "/api/sql/timeline/$uid"));
...
Future<TweetData> fetchTweetData(int pid) async {
  final getData = await http.get(Uri.http("https://23.254.244.168", "/api/sql/tweet/$pid"));
  if (getData.statusCode == 200) {
...
Future<bool> authenticate(String name, String pass) async {
  ...
  final getData = await http.get(Uri.http("https://23.254.244.168", "/api/sql/auth/$name/$pass"));
...
Future<bool> addUser(String name, String pass) async {
  ...
  final getData = await http.post(Uri.http("https://23.254.244.168", "/api/sql/newuser"), headers: {"Content-Type": "application/json"}, body: jsonEncode({"UNAME": name, "UFN": "", "ULN": "", "pass_hash": pass, "email": ""}));
...
Future<List<Tuple2<String, int>>> getAllUsers() async {
  final getData = await http.get(Uri.http("https://23.254.244.168", "/api/sql/allusers"));
...
Future follow(int uid) async {
  await http.post(Uri.http("https://23.254.244.168", "/api/sql/follow"), headers: {"Content-Type": "application/json"}, body: jsonEncode({"follower_id": Singleton().uid, "followed_id": uid}));
  Singleton().followingIds.add(uid);
}

Future unfollow(int uid) async {
  await http.post(Uri.http("https://23.254.244.168", "/api/sql/unfollow"), headers: {"Content-Type": "application/json"}, body: jsonEncode({"follower_id": Singleton().uid, "followed_id": uid}));
  Singleton().followingIds.remove(uid);
}

Future getFollowingIds() async {
  final getData = await http.get(Uri.http("https://23.254.244.168", "/api/sql/followers/${Singleton().uid}"));
...
Future makeTweet(String text) async {
  await http.post(Uri.http("https://23.254.244.168", "/api/sql/tweet"), headers: {"Content-Type": "application/json"}, body: jsonEncode({"UID": Singleton().uid, "text": text}));
}
